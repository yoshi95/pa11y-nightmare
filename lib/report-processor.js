...
function processMessages(options, messages) {
  return messages.map(processMessage).filter(function(message) {
    return isMessageWanted(options, message);
  });
}
...
function isMessageWanted(options, message) {
...
window.HTMLCS_processor = processMessages;
